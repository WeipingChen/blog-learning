...
Base.prototype.animate = function(obj) {
	for(var i = 0; i < this.elements.length; ++i) {
		...
		var attr = obj["attr"] == "x" ? "left" : obj["attr"] == "y" ? "top" :
				   ...
				   obj["attr"] == "o" ? "opacity" : "left";	//默认left 可选x、y
		...
		window.timer = setInterval(function(){
			...
			if(attr == "opacity") {
				...
				box.style[attr] = parseInt(getStyle(element, attr)) + step + "px";
				if(step == 0) {
					setTarget();
				}
			...
			// document.getElementById("aaa").innerHTML += parseInt(getStyle(element, attr)) + "<br />";
